...
function scrapeDetails(body, listingId, callback) {
    ...
    details = $('h2.public-detailed').html();

    //// Price, Address, MLS #, Type and style, year built

    try {
        someDetails = $('.public-detail-overview-b').contents();
        mls = someDetails[2].data.trim();
        proptype = someDetails[6].data;
        yearBuilt = someDetails[8].data;
        start = details.indexOf("<span>$");
    } catch (err) {
            ...
            try {
                ...
                var squareFeetPerFloor = $('h4:contains("Square Feet On Each Floor")').next().find('li').each(function(i, n) {
                    ...
                    if (sqftSplit[0] == 'Total') {
                        ...
                        newLevel.squareFeet = (' ' + newLevel.squareFeet).substring(1);
            ...
            try {
                ...
                var bathrooms = $('ul li:contains("Bathrooms")').next().children().each(function(i, n) {
                    ...
                    if (bathroomType == 'Full') {
                        bathroomType = 'bathsFull'
                    } else
                    if (bathroomType == 'Half') {
                        bathroomType = 'bathsHalf'
                    } else
                    {
